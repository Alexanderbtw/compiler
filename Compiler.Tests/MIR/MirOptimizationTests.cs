...
public sealed class MirOptimizationTests
{
    ...
    public void ControlFlowGraph_Builds_Diamond_Shape()
    {
        ...
        Assert.Equal(
            expected: 2,
            actual: cfg.GetSuccessors(entry).Count);
        ...
        Assert.Equal(
            expected: 2,
            actual: cfg.GetPredecessors(join).Count);
        Assert.Equal(
    ...
    public void O1_Folds_Constant_Branch_And_Removes_Unreachable_Blocks()
    {
        ...
        Assert.Contains(
            function.Blocks.Select(block => block.Terminator),
            terminator => terminator is BrCond);
    ...
    public void O1_Folds_Foldable_Builtins_And_Eliminates_Dead_Temps()
    {
        ...
        MirFunction function = TestUtils.BuildMir(
                src: src,
                level: MirOptimizationLevel.O1)
            .Functions
