...
public sealed class TrivialBlockCleanupPass : IMirOptimizationPass
{
        ...
        MirAnalysisManager analyses)
    {
        ...
        do
        {
            ...
            foreach (MirBlock block in function.Blocks.ToArray())
            {
                ...
                if (ReferenceEquals(
                        objA: block,
                        objB: target) || ReferenceEquals(
                        objA: target,
                        objB: entry) || cfg.GetPredecessors(target)
                        .Count != 1)
                {
                    continue;
                }
        ...
        MirBlock entry)
    {
        ...
        foreach (MirBlock block in function.Blocks)
        {
            if (ReferenceEquals(
                    objA: block,
                    objB: entry) || block.Instructions.Count != 0 || block.Terminator is not Br branch || ReferenceEquals(
                    objA: block,
                    objB: branch.Target))
            {
                continue;
