...
public sealed class VirtualMachine(VmModule mod, int stackSize = 1024)
{
    private readonly Stack<Frame> _call = new(32);

    private int _sp;
    private readonly Value[] _stack = new Value[stackSize];

    public Value Execute(string entry = "main", ReadOnlySpan<Value> args = default)
    {
        if (!mod.TryGetFunctionIndex(entry, out int i))
            throw new InvalidOperationException($"entry '{entry}' not found");
        Frame cur = NewFrame(i, args);
        while (true)
        {
            ...
            {
                // consts / locals
                case OpCode.LdcNull: Push(Value.Null); break;
                case OpCode.LdcI64: Push(Value.FromLong(ins.Imm)); break;
                case OpCode.LdcBool: Push(Value.FromBool(ins.Imm != 0)); break;
                case OpCode.LdcChar: Push(Value.FromChar((char)ins.Imm)); break;
                case OpCode.LdcStr: Push(Value.FromString(mod.StringPool[ins.Idx])); break;

                case OpCode.LdLoc: Push(cur.Locals[ins.A]); break;
                case OpCode.StLoc: cur.Locals[ins.A] = Pop(); break;
                ...
                case OpCode.Eq:
                {
                    Value r = Pop();
                    Value l = Pop();
                    Push(Value.FromBool(EqualsVal(l, r)));
                    break;
                }
                case OpCode.Ne:
                {
                ...
                case OpCode.LdElem:
                {
                    var idx = (int)Pop().AsLong();
                    VmArray arr = Pop().AsArr();
                    Push(arr[idx]);
                    break;
                }
                case OpCode.StElem:
                ...
                // control
                case OpCode.Br: cur.Pc = ins.A; break;
                case OpCode.BrTrue:
                    if (CoerceBool(Pop()).AsBool()) cur.Pc = ins.A;
                    break;
                case OpCode.Ret:
                {
                    Value ret = Pop();
                    if (_call.Count == 0) return ret;
                ...
                case OpCode.CallUser:
                {
                    ...
                    for (int idx = argc - 1; idx >= 0; --idx) argv[idx] = Pop();

                    _call.Push(cur); // save caller
                    cur = NewFrame(ins.A, argv);
                    break;
                }
                case OpCode.CallBuiltin:
                {
                    ...
                    Value r = BuiltinsVm.Invoke(name, argv);
                    Push(r);
                    break;
                }
    ...
    private static Value CoerceBool(Value v) => v.Tag switch
    {
        ValueTag.Bool => v,
        ...
        ValueTag.Array => Value.FromBool(v.AsArr().Length != 0),
    ...
    private static bool EqualsVal(Value a, Value b)
    {
        if (a.Tag != b.Tag)
        {
            ...
            if (a.Tag == ValueTag.I64 && b.Tag == ValueTag.Char) return a.I64 == b.C;
            if (a.Tag == ValueTag.Char && b.Tag == ValueTag.I64) return a.C == b.I64;
            return false;
        }
        return a.Tag switch
        {
            ...
            ValueTag.Array => ReferenceEquals(a.Ref, b.Ref),
            _ => Equals(a.Ref, b.Ref)
        };
    }

    private Frame NewFrame(int fnIdx, ReadOnlySpan<Value> args)
    {
    ...
    private void Push(Value v) { _stack[_sp++] = v; }
