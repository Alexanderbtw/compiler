...
{
    public VmModule Lower(MirModule m)
    {
        var vm = new VmModule();

        // заранее зарегистрируем все функции, чтобы знать индексы
        foreach (var f in m.Functions)
            vm.AddFunction(new VmFunction(f.Name, f.ParamRegs.Count));

        for (var i = 0; i < m.Functions.Count; i++)
            LowerFunction(m.Functions[i], vm.Functions[i], vm);

        return vm;
    }

    private static void LowerFunction(MirFunction f, VmFunction vf, VmModule vm)
    {
        // vreg -> local index
        ...
        foreach (var pr in f.ParamRegs)
        {
            if (!loc.ContainsKey(pr.Id)) loc[pr.Id] = NextLocal();
        ...
        void LdOp(MOperand op)
        {
            switch (op)
            {
                case Const c:
                    switch (c.Value)
                    {
                        case null: code.Add(new Instr { Op = OpCode.LdcNull }); break;
                        case long n: code.Add(new Instr { Op = OpCode.LdcI64, Imm = n }); break;
                        case bool b: code.Add(new Instr { Op = OpCode.LdcBool, Imm = b ? 1 : 0 }); break;
                        case char ch: code.Add(new Instr { Op = OpCode.LdcChar, Imm = ch }); break;
                        case string s:
                ...
                case VReg v:
                    if (!loc.TryGetValue(v.Id, out int idx))
                    {
                        idx = NextLocal();
                        loc[v.Id] = idx;
                    }
        ...
        for (var bIdx = 0; bIdx < f.Blocks.Count; bIdx++)
        {
            ...
            foreach (var ins in b.Instructions)
            {
                switch (ins)
                {
                    ...
                    {
                        // args
                        foreach (var a in cl.Args) LdOp(a);
                        if (vm.TryGetFunctionIndex(cl.Callee, out int fidx))
                        ...
                        if (cl.Dst is null) code.Add(new Instr { Op = OpCode.Pop });
                        else StDst(cl.Dst);
                        break;
                    }

                    default:
                        throw new NotSupportedException(ins.GetType().Name);
                }
            }
            ...
            else
            {
                ...
                {
                    case Ret r:
                        if (r.Value is null) code.Add(new Instr { Op = OpCode.LdcNull });
                        else LdOp(r.Value);
                        code.Add(new Instr { Op = OpCode.Ret });
                        break;
                    ...
                    default:
                        throw new NotSupportedException(b.Terminator.GetType().Name);
                }
            }
        }

        // фиксапы переходов
        foreach (var (pc, tgt) in brFixups)
            vf.Code[pc] = vf.Code[pc] with { A = label[tgt] };
        foreach (var (pcT, t, pcF, fblk) in condFixups)
        {
