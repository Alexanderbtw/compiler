...
public static class Builtins
{
    public static bool TryInvoke(string name, object?[] args, out object? result)
    {
        ...
        if (!CommonBuiltins.Table.TryGetValue(name, out List<BuiltinDescriptor>? cands) || cands.Count == 0)
        ...
        if (d is null) return false; // семантика должна была отловить, но на всякий случай

        switch (d.Lowering)
        {
            case BuiltinLoweringKind.CallRuntime:
                return TryInvokeRuntime(name, args, out result);
    ...
    private static bool TryInvokeRuntime(string name, object?[] args, out object? result)
    {
        result = null;
        switch (name)
        {
            ...
            case "array":
                if (args.Length == 1)
                {
                    ...
                    if (n < 0) throw new RuntimeException("array length must be non-negative");
                    result = new object?[n];
                    return true;
                }
                if (args.Length == 2)
                {
                    ...
                    for (var i = 0; i < n; i++) arr[i] = args[1];
                    result = arr;
                    return true;
                }
                return false;
